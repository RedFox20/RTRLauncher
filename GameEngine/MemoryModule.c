...
PMEMORYMODULE MemoryLoadLibrary(const void *data, const void* preferredBase)
{
    return MemoryLoadLibraryEx(data, preferredBase, _LoadLibrary, _GetProcAddress, _FreeLibrary, NULL);
}

PMEMORYMODULE MemoryLoadLibraryEx(const void *data, const void* preferredBase,
    ...
    void *userdata)
{
    ...
    PVOID imgBase = preferredBase ? preferredBase : (PVOID)old_header->OptionalHeader.ImageBase;
    code = (unsigned char *)VirtualAlloc(imgBase, old_header->OptionalHeader.SizeOfImage,
    ...
    consolef("MemoryModule imgBase at %p\n", code);
    // copy first 4KB so that the process has a valid Module Handle
...
FARPROC MemoryGetProcAddress(PMEMORYMODULE module, LPCSTR name)
{
    unsigned char *codeBase = module->codeBase;
    int idx=-1;
    DWORD i, *nameRef;
...
int MemoryCallEntryPoint(PMEMORYMODULE module)
{
...
int MemoryLoadString(PMEMORYMODULE module, UINT id, LPTSTR buffer, int maxsize)
{
    return MemoryLoadStringEx(module, id, buffer, maxsize, DEFAULT_LANGUAGE);
}

int MemoryLoadStringEx(PMEMORYMODULE module, UINT id, LPTSTR buffer, int maxsize, WORD language)
{
