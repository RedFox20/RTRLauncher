...
bool remote_dll_injector::inject_dllfile(void* targetProcessHandle, const char* filename)
{
	...
	auto func = shadow_string('L','o','a','d','l','i','b','r','a','r','y','A');
	FileInject.fnLoadLibraryA = (pLoadLibraryA)shadow_kernel_getproc(func.get());
	GetFullPathNameA(filename, sizeof FileInject.LibName, FileInject.LibName, 0);
...
inject_result remote_dll_injector::inject_dllimage(void* targetProcessHandle)
{
	...
	ManualInject.fnLoadLibraryA   = (pLoadLibraryA)shadow_kernel_getproc(shadow_string('L','o','a','d','l','i','b','r','a','r','y','A').get());
	ManualInject.fnGetProcAddress = (pGetProcAddress)shadow_kernel_getproc(shadow_string('G','e','t','P','r','o','c','A','d','d','r','e','s','s').get());
