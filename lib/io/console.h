...
template<class T> constream& operator<<(constream& os, const T& value)
{
...
template<int SIZE> constream& operator<<(constream& os, char (&str)[SIZE])
{
    console(str, strlen(str));
    return cons;
}
template<int SIZE> constream& operator<<(constream& os, const char (&str)[SIZE])
{
    console(str, SIZE - 1);
    return cons;
}
