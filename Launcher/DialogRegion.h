...
public:	
	...
	CDialogRegionT()
	{
		...
		HDC desktop = GetDC(0);
		CSize dpi = { GetDeviceCaps(desktop, LOGPIXELSX), GetDeviceCaps(desktop, LOGPIXELSY) };
	}
	...
	void GetDpiScaling(float& scaleX, float& scaleY)
	{
		HDC desktop = GetDC(0);
		...
		scaleX = GetDeviceCaps(desktop, LOGPIXELSX) / 96.0f;
		scaleY = GetDeviceCaps(desktop, LOGPIXELSY) / 96.0f;
		ReleaseDC(0, desktop);
	}
	...
	bool SetDialogRegion()
	{
		...
		//float scaleX, scaleY;
		//GetDpiScaling(scaleX, scaleY);
		int bmWidth = int(m_bmpInfo.bmWidth);
		int bmHeight = int(m_bmpInfo.bmHeight);
		...
		int iX  = (GetSystemMetrics(SM_CXVIRTUALSCREEN)) / 2 - (bmWidth / 2);
		int iY  = (GetSystemMetrics(SM_CYVIRTUALSCREEN)) / 2 - (bmHeight / 2);
		
		// Size the window to the size of bitmap
		iRet = pT->SetWindowPos(HWND_NOTOPMOST, iX, iY, bmWidth, bmHeight, NULL );
	...
	bool GenerateRegion()
	{
		...
		int bmWidth = int(m_bmpInfo.bmWidth);
		int bmHeight = int(m_bmpInfo.bmHeight);
		m_hRgn = CreateRectRgn(0, 0, int(bmWidth), int(bmHeight));
	...
	LRESULT OnEraseBkGnd(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		if (m_bIsRgnSet)
		{
			//float scaleX, scaleY;
			//GetDpiScaling(scaleX, scaleY);
			//int sWidth = int(scaleX * m_bmpInfo.bmWidth);
			//int sHeight = int(scaleY * m_bmpInfo.bmHeight);

			HDC hDC = (HDC) wParam;
			BitBlt(hDC, 0, 0, m_bmpInfo.bmWidth, m_bmpInfo.bmHeight, m_hdcBmp, 0, 0, SRCCOPY);

			//StretchBlt(hDC, 0, 0, sWidth, sHeight, m_hdcBmp, 0, 0, m_bmpInfo.bmWidth, m_bmpInfo.bmHeight, SRCCOPY);
		}
